...
def lambda_handler(event, context):
    ...
    responseBody = {"TEXT": {"body": json.dumps(handle_request(function, parameters))}}
    action_response = {
...
def handle_request(function, parameters):
    if function == "getCoordinates":
        ...
        return TimeFinder.get_current_time(
            parameters.get("latitude"), parameters.get("longitude")
        )
    elif function == "getCurrentWeather":
        return WeatherFinder.get_current_weather(
            parameters.get("latitude"), parameters.get("longitude")
        )
    else:
